...
class MetricCalc:
    """
    Hybrid Business Intelligence Service that merges high-volume facts from Cassandra
    with relational metadata from PostgreSQL for a unified view.
    """

    @staticmethod
    def _get_cassandra_repo():
        return get_repository("cassandra", shared=True)

    @staticmethod
    def _get_postgres_repo():
        return get_repository("postgres", shared=True)

    @classmethod
    def get_business_kpis(cls) -> Dict[str, Any]:
        """
        Calculates high-level KPIs based ONLY on real-time data from Cassandra.
        """
        c_repo = cls._get_cassandra_repo()
        settings = get_settings().extract.cassandra
        
        # 1. Fetch sales volume from Cassandra
        rows = c_repo.get_record(settings.default_table, columns=["product_id", "quantity", "price_at_sale", "ds"])
        if not rows:
        ...
        # 2. Extract Product Costs from PostgreSQL to compute actual Margin
        p_repo = cls._get_postgres_repo()
        products = p_repo.get_record("products", columns=["id", "unit_cost"])
        ...
        return {
            ...
            "conversion": "Pending", # Requires web session logs
            "margin": f"{margin:.1f}%",
    ...
    def get_revenue_breakdown(cls, category: str = "product") -> List[Dict[str, Any]]:
        """
        Performs a hybrid join between Cassandra sales facts and Postgres dimensions.
        ...
        # 1. Fetch sales facts from Cassandra
        rows = c_repo.get_record(settings.default_table, columns=["product_id", "quantity", "price_at_sale"])
        if not rows:
            return []
        ...
        # 2. Enrich with Product Names from PostgreSQL
        product_metadata = p_repo.get_record("products", columns=["id", "name"])
    ...
    def get_demand_trends(cls, scope: str = "week") -> List[Dict[str, Any]]:
        """
        Aggregates sales records from Cassandra into time-buckets for charting.
        """
        c_repo = cls._get_cassandra_repo()
        settings = get_settings().extract.cassandra
        
        rows = c_repo.get_record(settings.default_table, columns=["ds", "quantity"])
        if not rows:
            return []
        ...
        for _, row in resampled.iterrows():
            ...
            result.append({
                "name": date_str,
                "sales": int(row['sales']),
                "forecast": int(row['sales'] * 1.1) # Mock projected line
            })
    ...
    def get_user_stats(cls, category: str = "device") -> List[Dict[str, Any]]:
        """
        Mock user acquisition stats based on historical sales volume.
        """
        if category == "device":
            return [
    ...
    def get_inventory_status(cls, query: str = "") -> List[Dict[str, Any]]:
        """
        Combines logical metadata from Postgres with velocity calculated from Cassandra.
        Calculates risk and stock level based on REAL data. No simulations.
        """
        c_repo = cls._get_cassandra_repo()
        p_repo = cls._get_postgres_repo()
        settings = get_settings().extract.cassandra
        ...
        try:
            products_raw = p_repo.get_record("products", columns=["id", "name", "category_id", "status", "current_stock", "safety_stock"])
            categories = {row['id']: row['name'] for row in p_repo.get_record("categories")}
        except Exception as e:
            logger.error(f"Postgres fetch failed in inventory status: {e}")
            products_raw = []
            categories = {}

        # 2. Calculate stock velocity from Cassandra facts
        # Velocity = average quantity sold per transaction in this sample
        try:
            sales_rows = c_repo.get_record(settings.default_table, columns=["product_id", "quantity"])
            ...
            logger.error(f"Cassandra fetch failed in inventory status: {e}")
            velocity_map = {}
            all_sold_product_ids = set()

        # 3. Build a unified catalog of product IDs
        # We want to show everything in Postgres PLUS anything discovered in Sales
        ...
        for pid in all_product_ids:
            ...
            safety = int(prod.get('safety_stock', 10)) # fallback
            
            # Risk: High if stock is below safety threshold
